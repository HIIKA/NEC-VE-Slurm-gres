...
job_manager(stepd_step_rec_t *job)
{
	...
	if (!job->batch && (job->node_tasks > 1) &&
	    (job->accel_bind_type || job->tres_bind)) {
		uint64_t gpu_cnt, mic_cnt, nic_cnt, ve_cnt;
		...
		ve_cnt     = gres_plugin_step_count(job->step_gres_list, "ve");
		...
		if ((ve_cnt <= 1) || (ve_cnt == NO_VAL64))
			job->accel_bind_type &= (~ACCEL_BIND_CLOSEST_VE);
